...
if (scrollTopBar) {
  const ACCENT = { r: 169, g: 120, b: 100 };
  const DARK = { r: 62, g: 44, b: 39 };
  const clamp255 = (n) => Math.max(0, Math.min(255, Math.round(n)));
  const mix = (a, b, t) => clamp255(a + (b - a) * t);
  ...
  const fallbackRgbFromRoot = () => {
    const raw = getComputedStyle(document.documentElement).getPropertyValue("--bg").trim();
    const fromHex = hexToRgb(raw);
    if (fromHex) return fromHex;
    return parseCssRgb(getComputedStyle(document.body).backgroundColor) ?? { r: 247, g: 241, b: 238 };
  };

  const solidBackgroundRgb = (el) => {
  ...
  let chameleonRaf = 0;
  let reduceMotionScrollTimer = 0;
  const scheduleChameleon = () => {
    if (!scrollTopBar.classList.contains("is-visible")) return;
    if (chameleonRaf) return;
    chameleonRaf = requestAnimationFrame(() => {
      chameleonRaf = 0;
      const rect = scrollTopBar.getBoundingClientRect();
      const x = clampPx(rect.left + rect.width * 0.72, window.innerWidth - 1);
      const ys = [0.18, 0.42, 0.66, 0.88].map((t) =>
        clampPx(window.innerHeight * t, window.innerHeight - 1),
      );
      const samples = [];
      for (const y of ys) {
        ...
        for (const node of stack) {
          ...
          if (node === scrollTopBar || scrollTopBar.contains(node)) continue;
          const rgb = solidBackgroundRgb(node);
          if (rgb) {
            samples.push(rgb);
            break;
          }
      ...
      const topR = mix(r, 255, 0.42);
      const topG = mix(g, 255, 0.42);
      const topB = mix(b, 255, 0.42);
      const midR = mix(r, ACCENT.r, 0.28);
      const midG = mix(g, ACCENT.g, 0.28);
      const midB = mix(b, ACCENT.b, 0.28);
      const botR = mix(r, DARK.r, 0.38);
      const botG = mix(g, DARK.g, 0.38);
      const botB = mix(b, DARK.b, 0.38);

      const bg = `linear-gradient(165deg, rgba(${topR}, ${topG}, ${topB}, 0.82) 0%, rgba(${mix(r, 255, 0.22)}, ${mix(g, 255, 0.22)}, ${mix(b, 255, 0.22)}, 0.58) 22%, rgba(${midR}, ${midG}, ${midB}, 0.22) 50%, rgba(${botR}, ${botG}, ${botB}, 0.38) 100%)`;
      scrollTopBar.style.background = bg;

      const lum = relLuminance(r, g, b);
      const fg = lum < 0.52 ? "rgba(250, 246, 244, 0.96)" : "var(--text)";
      scrollTopBar.style.setProperty("--scroll-top-fg", fg);
    });
  };

  const onScrollOrResize = (event) => {
    scrollTopBar.classList.toggle("is-visible", window.scrollY > 400);
    if (reduceMotion()) {
      window.clearTimeout(reduceMotionScrollTimer);
      if (event?.type === "resize") {
        scheduleChameleon();
        return;
      }
      reduceMotionScrollTimer = window.setTimeout(scheduleChameleon, 140);
      return;
    }
    scheduleChameleon();
  };
  ...
  scrollTopBar.addEventListener("click", (event) => {
    event.stopPropagation();
...
if (fabDock && contactToggle && contactPanel) {
  ...
  window.addEventListener("click", (e) => {
    ...
    if (scrollTopBar?.contains(t)) return;
    requestCloseFabMenu();
  });
